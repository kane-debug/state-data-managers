...
const SocialLinks = ({ links }: { links: State['governor']['socialLinks'] }) => {
  ...
  if (!state || !state.info) {
    ...
    return (
      <div className="bg-white rounded-lg shadow-lg overflow-hidden p-6">
        <p className="text-red-600">Error: Invalid state data</p>
      </div>
    );
  }
      ...
      <div className="p-6">
        ...
        <div className="mb-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="font-medium text-gray-800">{state.governor.name}</p>
              <p className="text-gray-600">{state.governor.party}</p>
              <p className="text-gray-600">Since {state.governor.since}</p>
              <SocialLinks links={state.governor.socialLinks} />
            </div>
          </div>
        </div>

        <div className="mb-6">
          <h3 className="text-xl font-semibold text-gray-800 mb-3">Senators</h3>
            ...
            {state.senators.map((senator) => (
              <div key={senator.name} className="flex items-start gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600">Since {senator.since}</p>
                  <SocialLinks links={senator.socialLinks} />
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          <h3 className="text-xl font-semibold text-gray-800 mb-3">Representatives</h3>
            ...
            {state.representatives.map((rep) => (
              <div key={rep.name} className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-gray-600">{rep.district}</p>
                  <p className="text-gray-600">{rep.party}</p>
                </div>
                <SocialLinks links={rep.socialLinks} />
              </div>
            ))}
          </div>
        </div>
